...
def get_flame_speed_parallel(data_name, specifier, mech, T_inlet, P_inlet, func_, nProcs):
    t1 = time()
    fuel_list, ER, original_data, fuel_data = call_data_reader(specifier, data_name)
    ...
    gas = ct.Solution(mech)
    species_names = [i + '_b' for i in gas.species_names]
    data_parallel = parallel(mech, oneD_flame, nProcs, P_inlet, T_inlet, ER, fuel_list)
    ...
    solution   = [list(original_data.values[i]) + data_parallel[i] for i in range(len(original_data))]
    t2 = time()
...
def get_flame_speed(data_name, specifier, mech, T_inlet, P_inlet, func_):
    ...
    for i in range(len(fuel_list)):
        ...
        co.append(CO_ppmvd[idx_OH])
        nox.append(NOx_ppmvd[idx_OH])
        no_no2.append(NO_NO2_ppmvd[idx_OH])
        no.append(NO_ppmvd[idx_OH])   
        no2.append(NO2_ppmvd[idx_OH]) 
    ...
    t2 = time()
    print('Series: {0:.3f} seconds'.format(t2-t1))
    write(original_data, csv_file, excel_file, header, solution)
...
if __name__ == '__main__':
    ...
    nProcs = 6 #better use half of your total cores
    ...
    if func_ == "Equilibrium":
        ...
        get_flame_speed_parallel(data_name, specifier, mech, T_inlet, P_inlet, func_, nProcs)
